...
public class AiProcessingException extends RuntimeException {
  ...
  public static AiProcessingException invalidActionItemStatus(String status) {
    return new AiProcessingException(
        "The AI model returned an invalid action item status: '"
            + status
  ...
  public static AiProcessingException missingDefaultPromptTemplate(String templateName) {
    return new AiProcessingException(
        "No default prompt template ('"
            + templateName
            + "') found. Seed it via a Flyway migration or specify promptTemplateId explicitly.");
  }
}
